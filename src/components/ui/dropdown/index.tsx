...
export default function Dropdown({ className = '', label = 'Options', items = ['Option 1', 'Option 2'] }: DropdownProps) {
    ...
    return (
        <div className={`relative ${className}`}>
            <button onClick={() => setOpen(!open)} className="px-4 py-2 bg-white border rounded-sm shadow-sm hover:bg-gray-50">
                {label} ▼
            </button>
            {open && (
                <div className="absolute top-full left-0 mt-1 w-48 bg-white border rounded-sm shadow-lg z-10">
                    {items.map((item, i) => (
