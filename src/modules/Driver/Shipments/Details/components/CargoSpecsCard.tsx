...
import { Package, ShieldCheck, Thermometer, Layers, Weight, MapPin, Navigation } from 'lucide-react';
...
export const CargoSpecsCard: React.FC<Props> = ({ cargo, route }) => {
    return (
        <div className="bg-white dark:bg-[#1e2329] rounded-[4px] border border-slate-200/90 dark:border-slate-800 p-3.5 sm:p-4 shadow-2xs space-y-3 font-sans">
            <h2 className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                Cargo Specifications & Equipment Details
            </h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                ...
                {/* Special Handling / Security Status */}
                <div className="p-2.5 bg-slate-50 dark:bg-[#161a22] rounded-[4px] border border-slate-100 dark:border-slate-800">
                    <div className="text-[10.5px] font-semibold text-slate-400 flex items-center gap-1">
                        <ShieldCheck size={12} className="text-emerald-500" />
                        <span>Hazardous / ADR</span>
                    </div>
                    <div className="text-xs font-bold text-slate-900 dark:text-white mt-0.5">
                        {cargo.hazardous ? 'Yes (ADR Compliance Required)' : 'No (Standard Cargo)'}
                    </div>
                </div>
            </div>
        </div>
