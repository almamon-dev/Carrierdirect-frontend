/**
 * Driver NotificationRowActions Component (Ultra-Compact)
...
import { MoreVertical, ExternalLink, Check, Trash2, Copy, Clock } from 'lucide-react';
    ...
    return (
        <div className="relative flex items-center justify-end w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <Button
                variant="ghost"
                size="sm"
                className="h-6 w-6 p-0 rounded-[3px] text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer ml-auto flex items-center justify-center"
            ...
            {isOpen && createPortal(
                <>
                    ...
                    <div
                        className="fixed w-40 bg-white dark:bg-[#1e2329] rounded-[3px] shadow-xl border border-slate-200 dark:border-slate-700/80 py-1 z-[9999] animate-in fade-in zoom-in-95 duration-100 text-left font-sans text-xs"
                    ...
                    >
                        ...
                        {row.link && (
                            <button
                                type="button"
                                className="w-full text-left px-2.5 py-1.5 text-[11px] text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/80 flex items-center gap-2 transition-colors font-medium cursor-pointer"
                                onClick={() => {
                                    handleClose();
                                    if (row.unread) onMarkAsRead(row.id);
                                    navigate(normalizeNotifLink(row.link, 'driver'));
                                }}
                            >
                        ...
                        {row.unread ? (
                            ...
                            <button
                                ...
                                onClick={() => {
                                    handleClose();
                                    onMarkAsUnread(row.id);
                                }}
                            >
                                <Clock size={12} className="text-[#ff4a1f] shrink-0" />
