...
export default function PayoutStripeTab() {
  ...
  // 1-Click Instant Test Connect (Local Dev Testing)
  const handleTestConnect = async () => {
    setIsDevTesting(true);
    setNotification(null);
    try {
      await apiClient.post('/supplier/stripe/test-connect');
      setNotification({ type: 'success', message: '⚡ Test Stripe Account connected & stored in database!' });
  ...
  // Reset Test Account (Local Dev Testing)
  ...
  return (
    <form id="supplier-payouts-form" onSubmit={handleSave} className="space-y-4 font-sans antialiased w-full">
      ...
      {/* Stripe Connect - Compact Minimal Card */}
      <Card className="shadow-2xs border-slate-200 dark:border-slate-800 w-full rounded-[4px] overflow-hidden">
        <div
    className="p-3.5 sm:p-4 bg-white dark:bg-[#151921] flex flex-col gap-3">
          
          {/* Header Row */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-[4px] bg-[#635BFF]/10 text-[#635BFF] flex items-center justify-center shrink-0">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {loading ? (
                    <span className="text-[10px] text-slate-400 font-medium bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">Checking...</span>
                  ) : isVerified ? (
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                  Automated direct bank payout clearance powered by Stripe.
                </p>
              </div>
            </div>
          ...
          {/* Connected Details Grid - Minimal & Compact */}
          {isVerified && (
            <div className="pt-2.5 mt-1 border-t border-slate-100 dark:border-slate-800/80 grid grid-cols-1 sm:grid-cols-3 gap-2.5">
              <div
    className="p-2 bg-slate-50/70 dark:bg-[#1b2028] rounded-[3px] border border-slate-200/70 dark:border-slate-800">
    ...
    className="p-2 bg-slate-50/70 dark:bg-[#1b2028] rounded-[3px] border border-slate-200/70 dark:border-slate-800">
      ...
      {/* Payout Schedule */}
      <Card className="shadow-2xs border-slate-200 dark:border-slate-800 w-full rounded-[4px]">
        ...
        <CardContent className="p-3 sm:p-3.5">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            <label 
              ...
              className={`p-2.5 rounded-[4px] border cursor-pointer transition-all flex flex-col justify-between gap-1 ${
                payoutSchedule === 'daily' 
                  ? 'bg-orange-50/50 dark:bg-orange-950/20 border-[#ff4a1f] ring-1 ring-[#ff4a1f]/20' 
                  : 'bg-white dark:bg-[#181d24] border-slate-200 dark:border-slate-800 hover:bg-slate-50'
              }`}
            >
              ...
              <p className="text-[10.5px] text-slate-500 dark:text-slate-400 font-normal leading-tight">
      ...
      <Card className="shadow-2xs border-slate-200 dark:border-slate-800 w-full rounded-[4px]">
        ...
        <CardContent className="p-3 sm:p-3.5">
          <div className="flex flex-col gap-1 w-full sm:max-w-md">
            <label className="text-[11px] font-semibold text-slate-700 dark:text-slate-300">Standard Invoice Term *</label>
            <Select
              value={paymentTerms}
      ...
      <div className="flex items-center justify-between pt-1">
        <Button
          type="submit"
          disabled={isSaving}
          className="bg-[#FF4A1F] hover:bg-[#E03E15] text-white text-xs font-bold h-8.5 px-4 rounded-[4px] cursor-pointer flex items-center gap-1.5"
        >
