...
import { Check, CheckCircle2, ChevronDown, Clock, Copy, FileText, MoreVertical, Pin, Trash2, XCircle, Tag } from "lucide-react";
...
export const QuoteRequestBubbleCard: React.FC<QuoteRequestBubbleCardProps> = ({
    ...
    // Action permissions: supplier can only accept/counter if it is NOT their own submitted offer or if can_accept is explicitly granted
    const canAccept = Boolean((msg as any)?.can_accept || (activeNegotiation as any)?.can_accept);
...
Offer: €${initialOffer.toLocaleString()}
            ...
            <div className="relative w-full max-w-[390px]">
                <div
                    ...
                    className={`bg-white dark:bg-[#12161c] border border-slate-200/90 dark:border-slate-800 rounded-[6px] p-3.5 w-full text-left shadow-2xs space-y-2.5 font-sans transition-all duration-300 ${
                        highlightedMsgId === msg.id ? 'ring-2 ring-[#FF4A1F] ring-offset-2 scale-[1.01]' : ''
                    }`}
                >
                    {/* Header */}
                    ...
                    {/* Route & Distance */}
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-[11.5px]">
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/80">
                        <span className="text-[11.5px] font-medium text-slate-600 dark:text-slate-400">
                            {canAccept ? "Counter Offer Amount" : "Submitted Offer"}
                        </span>
                        <span className="text-[14px] font-black text-[#FF4A1F]">€ {initialOffer.toLocaleString()}</span>
                    </div>
                    ...
                    {isSuperseded ? (
                        ...
                        <div className="space-y-1.5 pt-1">
                            <div className="flex items-center gap-2">
                                <Button type="button" onClick={() => handleAcceptOffer(msg)} className="flex-1 h-7.5 rounded bg-emerald-600 hover:bg-emerald-700 text-white text-[11.5px] font-semibold cursor-pointer transition-all flex items-center justify-center gap-1">
