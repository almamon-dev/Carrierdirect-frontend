    ...
    return (
        <div className={`opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-0.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full shadow-xs px-1 py-0.5 text-slate-500 text-xs shrink-0 self-center ${isSent ? 'mr-1' : 'ml-1'}`}>
            <button
                type="button"
                onClick={handleCopy}
                className="p-1 hover:text-[#FF4A1F] rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer"
                title="Copy text"
            >
            ...
            <button
                type="button"
                onClick={onTogglePin}
                className={`p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer ${msg.isPinned ? 'text-amber-500' : 'hover:text-amber-500'}`}
            ...
            {isSent && onStartEdit && (
                <button
                    type="button"
                    onClick={onStartEdit}
                    className="p-1 hover:text-blue-500 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer"
                    title="Edit message"
                >
                    <TiEdit size={14} />
                </button>
            ...
            <button
                type="button"
                onClick={onDelete}
                className="p-1 hover:text-rose-500 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer"
