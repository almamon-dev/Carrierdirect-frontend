...
export const RecentQuotesList: React.FC<RecentQuotesListProps> = ({ quotes, isLoading = false }) => {
    ...
    className="bg-white dark:bg-[#1e2329] p-4 rounded-lg border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
            ...
            {/* Column Headers */}
            <div className="grid grid-cols-12 gap-1.5 text-[11px] font-bold text-slate-400 dark:text-slate-500 pb-2 border-b border-slate-100 dark:border-slate-800  tracking-wider">
                <span className="col-span-4 sm:col-span-2">Request</span>
                <span className="col-span-5 sm:col-span-3">Customer</span>
                <span className="hidden sm:block sm:col-span-2">Created</span>
                <span className="hidden sm:block sm:col-span-2 text-center">Rating</span>
                <span className="hidden sm:block sm:col-span-1 text-center">Orders</span>
                ...
                {isLoading ? (
                    <div className="space-y-3 py-2">
                        {[1, 2, 3, 4].map((i) => (
                            <div key={i} className="grid grid-cols-12 gap-1.5 items-center py-2 border-b border-dashed border-slate-200 dark:border-slate-800 last:border-0">
                                <div className="col-span-4 sm:col-span-2 h-3.5 w-10 bg-slate-200 dark:bg-slate-700/60 rounded animate-pulse" />
                                <div className="col-span-5 sm:col-span-3 h-3.5 w-16 bg-slate-200 dark:bg-slate-700/60 rounded animate-pulse" />
                                <div className="hidden sm:block sm:col-span-2 h-3.5 w-10 bg-slate-200 dark:bg-slate-700/60 rounded animate-pulse" />
                                <div className="hidden sm:block sm:col-span-2 h-3.5 w-7 mx-auto bg-slate-200 dark:bg-slate-700/60 rounded animate-pulse" />
                                <div className="hidden sm:block sm:col-span-1 h-3.5 w-5 mx-auto bg-slate-200 dark:bg-slate-700/60 rounded animate-pulse" />
                                <div className="col-span-3 sm:col-span-2 h-4 w-10 ml-auto bg-slate-200 dark:bg-slate-700/60 rounded animate-pulse" />
                            </div>
                        ))}
                    </div>
                ) : quotes.length === 0 ? (
                    ...
                    quotes.slice(0, 5).map((quote, idx) => (
                        ...
                        >
                            ...
                            <div className="col-span-5 sm:col-span-3 min-w-0 flex items-center gap-1.5">
                                {quote.customerAvatar ? (
                                    <img
                                        src={quote.customerAvatar}
                                        alt={quote.customerName || 'Customer'}
                                        className="w-5 h-5 rounded-full object-cover shrink-0 aspect-square border border-slate-200 dark:border-slate-700"
                                        onError={(e) => {
                                             (e.currentTarget as HTMLElement).style.display = 'none';
                                        }}
                                    />
                                ) : (
                                    <div className="w-5 h-5 rounded-full bg-orange-100 dark:bg-[#ff4a1f]/20 text-[#ff4a1f] flex items-center justify-center text-[10px] font-bold shrink-0 aspect-square border border-orange-200/50 dark:border-orange-500/20">
                            ...
                            <div className="hidden sm:flex sm:col-span-2 items-center justify-center gap-0.5">
